...
from Exp_bts_SyncIntegration_ConsistLoss.btsnet import BtsSDModel
from Exp_bts_SyncIntegration_ConsistLoss.bts_dataloader import BtsDataLoader
...
parser.add_argument('--lshapew',                   type=float, help='lambda in paper: [0, 1], higher value more focus on minimizing variance of error', default=1)
parser.add_argument('--consistw',                  type=float, help='weight for consistency loss', default=0.1)
...
def online_eval(model, normoptizer_eval, crfIntegrater, dataloader_eval, gpu, ngpus):
    ...
    if args.multiprocessing_distributed:
        ...
        dist.all_reduce(tensor=eval_measures_depth_garg_int, op=dist.ReduceOp.SUM, group=group)
    ...
    if not args.multiprocessing_distributed or gpu == 0:
        ...
        return eval_measures_depth_garg, eval_measures_depth_garg_int, eval_measures_shape
...
def compute_consistence_loss(normoptizer, pred_shape, intrinsic, pred_depth, pred_variance, pred_lambda):
    ...
    # fignorm = normoptizer.ang2normal(ang=pred_shape, intrinsic=intrinsic)
    # (tensor2rgb((fignorm + 1) / 2, viewind=viewind, isnormed=False)).show()
    # tensor2grad(depthMap_gradx_est * inboundh, pos_bar=0.1, neg_bar=-0.1, viewind=0).show()
    # tensor2grad(depthMap_gradx, pos_bar=0.1, neg_bar=-0.1, viewind=0).show()
    # tensor2grad(depthMap_grady_est * inboundv, pos_bar=0.1, neg_bar=-0.1, viewind=0).show()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
        ...
        eval_summary_writer_Depth_garg_int = SummaryWriter(os.path.join(args.log_directory, 'eval_Depth', "{}_{}_{}".format(args.model_name, 'garg', 'int')), flush_secs=30)
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    ...
                    fig_depth_norm = tensor2rgb((normoptizer.depth2norm(depthMap=pred_depth, intrinsic=K) + 1) / 2, isnormed=False)
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                time.sleep(0.1)
                model.eval()
                eval_measures_depth_garg, eval_measures_depth_garg_int, eval_measures_shape = online_eval(model=model, normoptizer_eval=normoptizer_eval, crfIntegrater=crfIntegrater, dataloader_eval=dataloader_eval, gpu=gpu, ngpus=ngpus_per_node)
                if eval_measures_depth_garg is not None:
                    ...
                    eval_summary_writer_Depth_garg_int.add_scalar('Depth_absrel_semidense_garg', eval_measures_depth_garg_int[1], int(global_step))
                    eval_summary_writer_Depth_garg_int.add_scalar('Depth_a1_semidense_garg', eval_measures_depth_garg_int[6], int(global_step))
                    ...
                    if epoch >= 10:
                        for kk in range(best_measures.shape[0]):
                            ...
                            if kk == 0 and eval_measures_shape[0] < best_measures[kk]:
                                ...
                                is_best = True
                            elif kk == 1 and eval_measures_depth_garg_int[1] < best_measures[kk]:
                                old_best_measure = best_measures[kk]
                                old_best_step = best_steps[kk]
                                best_measures[kk] = eval_measures_depth_garg_int[1]
                                best_steps[kk] = global_step
                                is_best = True
                            elif kk == 2 and eval_measures_depth_garg_int[6] > best_measures[kk]:
                                old_best_measure = best_measures[kk]
                                old_best_step = best_steps[kk]
                                best_measures[kk] = eval_measures_depth_garg_int[6]
                                best_steps[kk] = global_step
                                is_best = True
                            if is_best:
