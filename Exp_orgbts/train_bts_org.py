...
def online_eval(model, dataloader_eval, gpu, ngpus):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        if args.garg_crop or args.eigen_crop:
            gt_width, gt_height = eval_sample_batched['size'][0].cpu().numpy()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
        ...
        eval_summary_writer = SummaryWriter(os.path.join(args.log_directory, 'eval_Depth', "{}_{}".format(args.model_name, 'garg')), flush_secs=30)
