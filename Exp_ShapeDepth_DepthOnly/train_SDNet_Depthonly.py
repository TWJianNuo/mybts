...
def main_worker(gpu, ngpus_per_node, args):
    ...
    global_step = 0
    best_measures = np.zeros([2])
    best_measures[0] = 1e3
    best_measures[1] = 0
    best_steps = np.zeros([2])
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
        ...
        eval_summary_writer = SummaryWriter(os.path.join(args.log_directory, 'eval_Depth', args.model_name), flush_secs=30)
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            loss = compute_depth_loss(outputs=outputs, depth_gt=depth_gt)
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    writer.add_scalar('depthloss', loss, global_step)
                    ...
                    figoveiew = np.concatenate([np.array(fig_rgb), np.array(fig_gt), np.array(fig_depth)], axis=0)
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                ...
                print("Best Depth abserl: %f, at step %d" % (best_measures[0], best_steps[9]))
                ...
                if epoch >= 10:
                    for kk in range(best_measures.shape[0]):
                        ...
                        if kk == 0 and eval_measures_depth[1] < best_measures[0]:
                            ...
                            is_best = True
                        elif kk == 1 and eval_measures_depth[6] > best_measures[1]:
                            old_best_measure = best_measures[kk]
                            old_best_step = best_steps[kk]
