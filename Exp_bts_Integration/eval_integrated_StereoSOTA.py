...
def online_eval(model, normoptizer_eval, crfIntegrater, dataloader_eval, gpu, vlsroot):
    ...
    for idx, eval_sample_batched in enumerate((dataloader_eval.data)):
        with torch.no_grad():
            ...
            predfilename = dataloader_eval.testing_samples.filenames[idx].split(' ')[0]
            comps = predfilename.split('/')
            predfilename = "{}_{}".format(comps[1], comps[4])
            pred_depth = pil.open(os.path.join('/media/shengjie/disk1/visualization/LEAStereo_pred', predfilename))
            ...
            _, _, height, width = pred_depth.shape
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    vlsroot = os.path.join('/media/shengjie/disk1/visualization/btspred', "SOTA_Stereo_integrated")
