...
def online_eval(model, normoptizer_eval, crfIntegrater, dataloader_eval, gpu, ngpus):
    ...
    if not args.multiprocessing_distributed or gpu == 0:
        ...
        return eval_measures_depth, eval_measures_depth_garg, eval_measures_depth_garg_int, eval_measures_shape
    return None
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                time.sleep(0.1)
                model.eval()
                eval_measures_depth, eval_measures_depth_garg, eval_measures_depth_garg_int, eval_measures_shape = online_eval(model=model, normoptizer_eval=normoptizer_eval, crfIntegrater=crfIntegrater, dataloader_eval=dataloader_eval, gpu=gpu, ngpus=ngpus_per_node)
