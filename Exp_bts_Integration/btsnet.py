...
import math
from Exp_bts_NoIntegration.shapenet import ResnetEncoder, ShapeDecoder
import numpy as np
...
class local_planar_guidance(nn.Module):
    ...
    def forward(self, plane_eq, focal):
        ...
        denumerator = n1 * u + n2 * v + n3
        sign_denumerator = denumerator.sign()
        denumerator = denumerator.abs().clamp_(min=1e-3, max=np.inf)
        denumerator = denumerator * sign_denumerator
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, num_features=512):
        ...
        self.get_variance = torch.nn.Sequential(nn.Conv2d(num_features // 16, 1, 3, 1, 1, bias=False), nn.Sigmoid())
        self.get_lambda = torch.nn.Sequential(nn.Conv2d(num_features // 16, 1, 3, 1, 1, bias=False), nn.Sigmoid())
    def forward(self, features, focal):
        ...
        pred_variance = self.get_variance(iconv1) * (self.params.clipvariance + 1)
        pred_lambda = self.get_lambda(iconv1)

        outputs['depth_8x8_scaled'] = depth_8x8_scaled
        outputs['depth_4x4_scaled'] = depth_4x4_scaled
        outputs['depth_2x2_scaled'] = depth_2x2_scaled
        outputs['reduc1x1'] = reduc1x1
        outputs['final_depth'] = final_depth
        outputs['pred_variance'] = pred_variance
        outputs['pred_lambda'] = pred_lambda
        return outputs
...
class BtsSDModel(nn.Module):
    def __init__(self, params):
        super(BtsSDModel, self).__init__()
        self.params = params
        self.depth_encoder = encoder(params)
        self.depth_decoder = bts(params, self.depth_encoder.feat_out_channels, params.bts_size)

        self.shape_encoder = ResnetEncoder(50, pretrained=True)
        self.shape_decoder = ShapeDecoder(self.shape_encoder.num_ch_enc, num_output_channels=2)

    def forward(self, x, focal):
        outputs = dict()
        skip_feat = self.depth_encoder(x)
        outputs.update(self.depth_decoder(skip_feat, focal))
        outputs.update(self.shape_decoder(self.shape_encoder(x)))
        return outputs
