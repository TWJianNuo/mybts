...
from Exp_demon.bts_dataloader_mvsonly import BtsDataLoader
import numpy as np
...
parser.add_argument('--dist_url',                  type=str,   help='url used to set up distributed training', default='tcp://127.0.0.1:2234')
...
def online_eval(model, dataloader_eval, gpu, ngpus):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        with torch.no_grad():
            ...
            pred_depth = align_mvs(pred_depth, torch.autograd.Variable(gt_depth.cuda(gpu, non_blocking=True)), eval_sample_batched['entry'])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            entry = sample_batched['entry']

            lpg8x8, lpg4x4, lpg2x2, reduc1x1, depth_est = model(image)

            depth_est = align_mvs(depth_est, depth_gt, entry)
            mask = (depth_gt > 0) * (depth_gt < args.max_depth)

            for kk in range(len(entry[0])):
                if entry[0][kk].split(' ')[0].split('_')[0] == 'mvs':
                    mask[kk] = (depth_gt[kk] > 0) * (depth_gt[kk] < 100)
