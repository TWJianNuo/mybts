...
def online_eval(model, normoptizer_eval, dataloader_eval, gpu, vlsroot):
    ...
    for idx, eval_sample_batched in enumerate((dataloader_eval.data)):
        with torch.no_grad():
            ...
            figoveiew = np.concatenate([np.array(fig_depth), np.array(fig_depth_norm)], axis=1)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model.eval()
    online_eval(model=model, normoptizer_eval=normoptizer_eval, dataloader_eval=dataloader_eval, gpu=gpu, vlsroot=vlsroot)
