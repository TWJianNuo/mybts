...
parser.add_argument('--startepoch',                type=int,   help="mounted to depth loss", default=15)
parser.add_argument('--endepoch',                  type=int,   help="mounted to depth loss", default=25)
...
def online_eval(model, normoptizer_eval, crfIntegrater, dataloader_eval, gpu, ngpus):
    ...
    if not args.multiprocessing_distributed or gpu == 0:
        ...
        print('{:7.3f}'.format(eval_measures_lateralre[8]))
        return eval_measures_shape, eval_measures_depth, eval_measures_lateralre
    return None
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    ...
                    fig_lambda = tensor2disp(outputs[('lambda', 0)], vmax=1, viewind=viewind)
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                time.sleep(0.1)
                model.eval()
                eval_measures_shape, eval_measures_depth, eval_measures_lateralre = online_eval(model=model, normoptizer_eval=normoptizer_eval, crfIntegrater=crfIntegrater, dataloader_eval=dataloader_eval, gpu=gpu, ngpus=ngpus_per_node)
