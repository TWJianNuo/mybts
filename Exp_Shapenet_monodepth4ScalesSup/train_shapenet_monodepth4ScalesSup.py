...
from Exp_Shapenet_monodepth4ScalesSup.shapedataset import KittiShapeDataLoader, KittiShapeDataset
from Exp_Shapenet_monodepth4ScalesSup.shapenet import ShapeNet
from util import *
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            for idx in range(3, -1, -1):
                key = ('disp', idx)
                pred_shape = outputs[key]
                pred_shape = F.interpolate(pred_shape, [kbcroph, kbcropw], mode='bilinear', align_corners=True)
                scaleloss, hloss, vloss, inbdh, inbdv = normoptizer.intergrationloss_ang(ang=pred_shape, intrinsic=K, depthMap=depth_gt)
                loss += scaleloss
            loss = loss / 4
            loss.backward()
            optimizer.step()
