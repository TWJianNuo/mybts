...
def export(args, evaluation_entries):
    ...
    with torch.no_grad():
        for t_idx, entry in enumerate(tqdm(evaluation_entries)):
            ...
            imgpath = os.path.join(args.data_path, 'raw', seq, 'image_02', 'data', "{}.png".format(str(index).zfill(10)))
            calib_dir = os.path.join(args.data_path, 'raw', seq.split('/')[0])
...
if __name__ == '__main__':
    eval_entries = read_splits_mapping()
