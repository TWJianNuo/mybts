...
def export(gpuid, model, args, ngpus_per_node, evaluation_entries):
    """Test function."""
    ...
    with torch.no_grad():
        for t_idx, entry in enumerate(tqdm(evaluation_entries[stidx : edidx])):
            ...
            predread_np_orgsize = np.zeros([gt_height, gt_width])
            predread_np_orgsize[top_margin:top_margin + 352, left_margin:left_margin + 1216] = depth_est
            predread_np_orgsize = (predread_np_orgsize * 256.0).astype(np.uint16)

            Image.fromarray(predread_np_orgsize).save(export_path)
